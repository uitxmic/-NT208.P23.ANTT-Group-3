...
CREATE PROCEDURE fn_update_voucher_id_after_buying(IN v_userId INT, IN v_voucherId INT, IN v_postId INT)
BEGIN
	DECLARE p_voucher_code TEXT;
    SELECT VoucherCode FROM 
    Voucher V JOIN Post P
    ON V.VoucherId = P.VoucherId
    WHERE P.VoucherId = v_voucherId AND P.PostId = v_postId
    LIMIT 1
    INTO p_voucher_code; 
...
CALL fn_update_voucher_id_after_buying(2,1,4);